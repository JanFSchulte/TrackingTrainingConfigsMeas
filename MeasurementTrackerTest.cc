...
void MeasurementTrackerTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(auto it : compLayers){
    ...
    MeasurementDetWithData det = measurementTrackerH->idToDet(detWithState.front().first->geographicalId());
	...
	det.fastMeasurements(detWithState.front().second, tsos, ANprop, estimator);  //Second TSOS is not used
